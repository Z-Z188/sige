...
def main_recon_mp4():
    ...
    out_path = os.path.join(base_dir, "recon.mp4")
...
@torch.no_grad()
def main_sdedit_two_images(args: argparse.Namespace):
    from chunk_utils import iter_5_4_4_chunks
    from mask_utils import compute_sdedit_masks
    from sige_model3d import SIGEAutoencoder3d
    from video_utils import load_image_as_tensor, repeat_image_to_video, save_video_tensor

    device = torch.device(args.device)
    dtype = torch.bfloat16 if device.type == "cuda" else torch.float32

    base_dir = os.path.dirname(__file__)
    init_img_path = args.init_img or os.path.join(base_dir, "1_origin.png")
    edited_img_path = args.edited_img or os.path.join(base_dir, "1_edit.png")

    resize_hw = None
    if args.resize_h is not None and args.resize_w is not None:
        resize_hw = (int(args.resize_h), int(args.resize_w))

    init_img = load_image_as_tensor(init_img_path, resize_hw=resize_hw, device=device, dtype=torch.float32)
    edited_img = load_image_as_tensor(edited_img_path, resize_hw=resize_hw, device=device, dtype=torch.float32)

    masks_enc, masks_dec, diff_mask = compute_sdedit_masks(init_img, edited_img, min_res=(4, 4))
    edit_ratio = diff_mask.float().mean().item() * 100.0
    print(f"[Mask] Edit ratio: {edit_ratio:.2f}%")

    init_video = repeat_image_to_video(init_img.to(dtype=dtype), args.num_frames)
    edited_video = repeat_image_to_video(edited_img.to(dtype=dtype), args.num_frames)

    model = SIGEAutoencoder3d(
        ch=args.ch,
        in_channels=3,
        out_channels=3,
    ...
    # ---------- Encoder: full(init) -> sparse(edited) ----------
    encoder.clear_cache()
    encoder.clear_stream_cache()
    encoder.set_mode("full")
    init_latents = []
    for chunk_id, chunk in enumerate(iter_5_4_4_chunks(init_video)):
        encoder.set_cache_id(chunk_id)
        init_latents.append(encoder(chunk))
    init_latent = torch.cat(init_latents, dim=2)

    encoder.clear_stream_cache()
    encoder.set_mode("sparse")
    encoder.set_masks(masks_enc)
    edited_latents = []
    for chunk_id, chunk in enumerate(iter_5_4_4_chunks(edited_video)):
        encoder.set_cache_id(chunk_id)
        edited_latents.append(encoder(chunk))
    edited_latent = torch.cat(edited_latents, dim=2)

    # ---------- Decoder: full(init_latent) -> sparse(edited_latent) ----------
    decoder.clear_cache()
    decoder.clear_stream_cache()
    decoder.set_mode("full")
    recon_init_chunks = []
    for chunk_id, z_chunk in enumerate(iter_5_4_4_chunks(init_latent)):
        decoder.set_cache_id(chunk_id)
        recon_init_chunks.append(decoder(z_chunk))
    recon_init = torch.cat(recon_init_chunks, dim=2)

    decoder.clear_stream_cache()
    decoder.set_mode("sparse")
    decoder.set_masks(masks_dec)
    recon_edited_chunks = []
    for chunk_id, z_chunk in enumerate(iter_5_4_4_chunks(edited_latent)):
        decoder.set_cache_id(chunk_id)
        recon_edited_chunks.append(decoder(z_chunk))
    recon_edited = torch.cat(recon_edited_chunks, dim=2)

    out_dir = args.out_dir or base_dir
    os.makedirs(out_dir, exist_ok=True)

    save_video_tensor(os.path.join(out_dir, "sdedit_recon_init.mp4"), recon_init.clamp(-1, 1), fps=args.fps)
    save_video_tensor(os.path.join(out_dir, "sdedit_recon_edited.mp4"), recon_edited.clamp(-1, 1), fps=args.fps)
    print(f"[Saved] {os.path.join(out_dir, 'sdedit_recon_init.mp4')}")
    print(f"[Saved] {os.path.join(out_dir, 'sdedit_recon_edited.mp4')}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--task", default="sdedit", choices=["recon_mp4", "sdedit"])
    parser.add_argument("--device", default="cuda:0" if torch.cuda.is_available() else "cpu")

    # sdedit-like settings
    parser.add_argument("--init-img", dest="init_img", type=str, default=None)
    parser.add_argument("--edited-img", dest="edited_img", type=str, default=None)
    parser.add_argument("--num-frames", type=int, default=100)
    parser.add_argument("--resize-h", type=int, default=480)
    parser.add_argument("--resize-w", type=int, default=832)
    parser.add_argument("--fps", type=int, default=16)
    parser.add_argument("--out-dir", type=str, default=None)

    # model hyper-params (toy model; no pretrained weights here)
    parser.add_argument("--ch", type=int, default=64)
    parser.add_argument("--z-channels", type=int, default=4)
...
if __name__ == "__main__":
    args = parse_args()
    if args.task == "recon_mp4":
        main_recon_mp4()
    else:
        main_sdedit_two_images(args)
...
def get_warped_and_mask(flow_model,
    ...
    # 是否使用“像素一致性检查(pixel_consistency Check )”来进一步检测遮挡（occlusion）
    # 默认的遮挡检测只依赖 前后光流一致性（forward-backward consistency），
    
    if pixel_consistency:  
        # 像素一致性判断只需要“一次映射（warp）”，不需要走完整的几何闭环。
